...
const multiCategories=[
  ...
  useEffect(() => {
    const getMovies = async () => {
      try {
        ...
        const nationQuery= nationCategory? `&repNationCd=${nationCategory}` : ''
        const multiQuery=multiCategory?`&multiMovieYn=${multiCategory}` :''
        let response = await axios.get(
          `http://kobis.or.kr/kobisopenapi/webservice/rest/boxoffice/searchDailyBoxOfficeList.json?key=a5ff207b0075a51ed12c42d6c4b67218&targetDt=20220905${nationQuery}${multiQuery}`,
        )
  ...
  return (
    ...
    <ul>
      {nationCategories.map((i,index)=>{
        return(
              <li key={index}><button onClick={()=>{setNationCategories(i.text)}}>{i.name}</button></li>         
        )
      })}
    </ul>
    <ul>
      {multiCategories.map((i,index)=>{
        return(
              <li key={index}><button onClick={()=>{setMultiCategorie(i.text)}}>{i.name}</button></li>         
        )
      })}
    </ul>
